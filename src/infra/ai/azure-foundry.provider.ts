...
} from "./transaction-category-classifier";
...
export class AzureFoundryProvider implements AiProvider {
  ...
  constructor(private readonly config: ConfigService) {
    ...
    this.deployment =
      config.get<string>("azureOpenAi.deployment") ?? "gpt-4o-mini";
  }
  ...
  private describeUserProfile(profile: UserProfile): string {
    ...
    if (profile.incomeType)
      parts.push(`Fuente de ingresos: ${profile.incomeType}.`);
    if (profile.averageMonthlyIncome)
      parts.push(
        `Ingreso mensual promedio: S/${profile.averageMonthlyIncome.toFixed(2)}.`,
      );
  ...
  async predictExpenses(context: SpendingContext): Promise<PredictionResult> {
    if (!this.isConfigured) {
      this.logger.warn(
        "Azure OpenAI not configured — using statistical fallback for expense prediction",
      );
...
Responde SOLO con JSON válido con esta estructura exacta:
{
  ...
  "narrative": "string en español explicando la predicción en 2-3 oraciones"
      ...
      this.logger.error(
        "Azure AI expense prediction failed, using fallback",
        err,
      );
      return this.statisticalPrediction(context);
    }
  ...
  async generateRecommendations(
    ...
    if (!this.isConfigured) {
      this.logger.warn(
        "Azure OpenAI not configured — using rule-based recommendations",
      );
...
Devuelve una de estas categorias exactas en ingles:
Food, Transportation, Housing, Utilities, Health, Entertainment, Shopping, Subscriptions, Cravings, Savings, Scholarship, Part-time work, Family, Freelance, Other.

Guia rapida:
- Food: supermercado, menu, restaurante, almuerzo/cena.
- Cravings: snacks, bebidas, galletas, cafe, delivery pequeno, postres, antojos.
- Shopping: ropa, zapatos, accesorios, libros/materiales.
- Subscriptions: Netflix, Spotify, apps o planes mensuales.
- Scholarship: becas, Pronabec, subvenciones de estudio.
- Part-time work: sueldo, salario, trabajo part-time o medio tiempo.
- Family: apoyo familiar, mesada, dinero de padres o familiares.
- Freelance: honorarios, clientes, proyectos independientes, cachuelos.

Responde SOLO con JSON: {"categoryName": "Food|Transportation|Housing|Utilities|Health|Entertainment|Shopping|Subscriptions|Cravings|Savings|Scholarship|Part-time work|Family|Freelance|Other", "confidence": number_0_to_1}`;

      const userPrompt = `Descripción: "${description}", Monto: S/${amount}`;
      const raw = await this.callAzure(systemPrompt, userPrompt);
      const parsed = JSON.parse(raw) as Partial<ClassificationResult>;
      return normalizeClassificationResult(parsed, description, amount);
    } catch (err) {
  ...
  private statisticalPrediction(context: SpendingContext): PredictionResult {
    ...
    const sorted = [...context.months].sort((a, b) =>
  ...
  async generatePersonalizedQuiz(
    ...
    if (!this.isConfigured) {
      this.logger.warn(
        "Azure OpenAI not configured — using fallback personalized quiz",
      );
    ...
    try {
      ...
      const latestMonth = context.months.sort((a, b) =>
        b.period.localeCompare(a.period),
      )[0];
...
Genera 5 preguntas personalizadas de educación financiera enfocadas en sus hábitos de gasto.`;
      ...
      this.logger.error(
        "Azure AI personalized quiz failed, using fallback",
        err,
      );
      return { questions: this.fallbackPersonalizedQuestions(language) };
    }
  ...
  private fallbackPersonalizedQuestions(
    language: string,
  ): PersonalizedQuizQuestion[] {
    const isEs = language === "es";
    if (isEs) {
      return [
        {
          text: "¿Qué porcentaje de ingresos recomienda la regla 50/30/20 para necesidades básicas?",
          options: ["A) 20%", "B) 30%", "C) 50%", "D) 70%"],
          correctAnswer: "C) 50%",
          difficulty: "BEGINNER" as PersonalizedQuizDifficulty,
        },
        {
          text: "¿Qué se recomienda hacer primero al recibir tu salario?",
          options: [
            "A) Gastar en entretenimiento",
            "B) Pagar deudas",
            "C) Apartar el ahorro antes de gastar",
            "D) Invertir en bolsa",
          ],
          correctAnswer: "C) Apartar el ahorro antes de gastar",
          difficulty: "BEGINNER" as PersonalizedQuizDifficulty,
        },
        {
          ...
          correctAnswer: "B) 3 a 6 meses",
          difficulty: "INTERMEDIATE" as PersonalizedQuizDifficulty,
        },
        {
          text: '¿Cuál de estas opciones es una deuda "productiva"?',
          options: [
            "A) Deuda en tarjeta por ropa",
            "B) Préstamo para estudios universitarios",
            "C) Crédito para vacaciones",
            "D) Deuda por artículos de lujo",
          ],
          correctAnswer: "B) Préstamo para estudios universitarios",
          difficulty: "INTERMEDIATE" as PersonalizedQuizDifficulty,
        },
        {
          text: "¿Qué es el interés compuesto?",
          options: [
            "A) Interés solo sobre el capital inicial",
            "B) Interés calculado sobre capital más intereses acumulados",
            "C) Un préstamo sin intereses",
            "D) El interés mensual del banco",
          ],
          correctAnswer:
            "B) Interés calculado sobre capital más intereses acumulados",
          difficulty: "ADVANCED" as PersonalizedQuizDifficulty,
        },
      ];
    }
    return [
      {
        text: "What percentage of income does the 50/30/20 rule recommend for basic needs?",
        options: ["A) 20%", "B) 30%", "C) 50%", "D) 70%"],
        correctAnswer: "C) 50%",
        difficulty: "BEGINNER" as PersonalizedQuizDifficulty,
      },
      {
        text: "What should you do first when you receive your salary?",
        options: [
          "A) Spend on entertainment",
          "B) Pay debts",
          "C) Save before spending",
          "D) Invest in stocks",
        ],
        correctAnswer: "C) Save before spending",
        difficulty: "BEGINNER" as PersonalizedQuizDifficulty,
      },
      {
        ...
        correctAnswer: "B) 3 to 6 months",
        difficulty: "INTERMEDIATE" as PersonalizedQuizDifficulty,
      },
      {
        text: 'Which of these is considered "productive debt"?',
        options: [
          "A) Credit card debt for clothing",
          "B) Student loan for university",
          "C) Loan for vacation",
          "D) Debt for luxury items",
        ],
        correctAnswer: "B) Student loan for university",
        difficulty: "INTERMEDIATE" as PersonalizedQuizDifficulty,
      },
      {
        text: "What is compound interest?",
        options: [
          "A) Interest only on the initial capital",
          "B) Interest calculated on capital plus accumulated interest",
          "C) An interest-free loan",
          "D) The monthly bank interest",
        ],
        correctAnswer:
          "B) Interest calculated on capital plus accumulated interest",
        difficulty: "ADVANCED" as PersonalizedQuizDifficulty,
  ...
  private ruleBasedRecommendations(
    ...
    if (context.months.length === 0) {
      recs.push({
        type: "SAVINGS",
        message:
          "Comienza a registrar tus gastos para recibir recomendaciones personalizadas.",
        suggestedAction: "Registra al menos una transacción hoy.",
      });
      return recs;
    }

    const latest = context.months.sort((a, b) =>
      b.period.localeCompare(a.period),
    )[0]!;
    const savingsRate =
    ...
    if (savingsRate < 0.2) {
      recs.push({
        ...
        suggestedAction:
          "Revisa tus gastos de entretenimiento y reduce al menos un 10%.",
      });
    }
    ...
    recs.push({
      ...
      suggestedAction:
        "Establece un presupuesto para tu categoría de mayor gasto.",
    });
